...
public:
    ...
    Texture* LoadTextureTga2(RenderDevice* ren, File* f, unsigned char alpha);
    ...
    pthread_mutex_t texDataLock;
    char *texData[3];
    
    void handleDataIn(char **inData);
...
void dataInCallback(char **sentData, void *data) {
    OculusWorldDemoApp *app = (OculusWorldDemoApp *) data;
    app->handleDataIn(sentData);
}
      ...
      TextScreen(Text_None)
{
    ...
    pthread_mutex_init(&texDataLock, NULL);
    setHackCallback(dataInCallback);
    runHackThread(this);
}
...
void OculusWorldDemoApp::handleDataIn(char **inData) {
    std::cout << "GOT DATA" << std::endl;
    pthread_mutex_lock(&texDataLock);
    for(int i = 0; i < 3; i++) {
        free(texData[i]);
        texData[i] = inData[i];
    }
    pthread_mutex_unlock(&texDataLock);
}
...
Texture* OculusWorldDemoApp::LoadTextureTga2(RenderDevice* ren, File* f, unsigned char alpha)
{
    f->SeekToBegin();
...
//Texture* OculusWorldDemoApp::LoadTextureTga2(RenderDevice* ren)
//{
////    f->SeekToBegin();
//    
//    int width = 640;
...
//        for (int x = 0; x < width; x++)
//        {
//            imgdata[y*bpl+x*4+0] = 255; //buf[2];
//            imgdata[y*bpl+x*4+1] = 255; //buf[1];
//            imgdata[y*bpl+x*4+2] = 255; //buf[0];
//            imgdata[y*bpl+x*4+3] = 0;
//        }
//    }
////                break;
////            case 32:
...
void OculusWorldDemoApp::PopulatePreloadScene()
{
    ...
    if (imageFile->IsValid()) {
        pthread_mutex_lock(&texDataLock);
        imageTex = *LoadTextureTga(pRender, imageFile, texData);
        pthread_mutex_unlock(&texDataLock);
    }
